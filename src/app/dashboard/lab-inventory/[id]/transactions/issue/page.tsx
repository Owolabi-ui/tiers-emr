...
export default function IssueLabStockPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      setLoading(true);
      setError(null);
      await labInventoryApi.issueStock({
        item_id: id,
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-[#5b21b6] mt-2">Issue Stock</h1>
      </div>
      ...
      <form onSubmit={onSubmit} className="rounded-xl border bg-white dark:bg-neutral-900 p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input placeholder="Issue / Ref No" className="h-10 rounded-lg border px-3" value={form.voucher_number} onChange={(e) => setForm({ ...form, voucher_number: e.target.value })} />
          <input placeholder="Issued to" className="h-10 rounded-lg border px-3" value={form.counterparty} onChange={(e) => setForm({ ...form, counterparty: e.target.value })} />
          ...
          <input type="number" step="0.001" required placeholder="Quantity issued" className="h-10 rounded-lg border px-3" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} />
        </div>
